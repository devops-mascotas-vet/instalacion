...
def clone_bench_repo(args):
    ...
    if os.path.exists(tmp_bench_repo):
        log(
            "Not cloning already existing Bench repository at {tmp_bench_repo}".format(
        ...
        log(
            "--without-bench-setup specified, clone path is: {clone_path}".format(
        ...
        log(
            "Cloning bench repository branch {branch} into {clone_path}".format(
...
def parse_commandline_args():
    ...
    parser.add_argument(
        ...
        action="store",
        default="site1.local",
        help="Specify name for your first ERPVet site",
    )
    parser.add_argument(
        "--without-site",
        dest="without_site",
    ...
    parser.add_argument(
        ...
        action="store",
        default="https://github.com/devops-mascotas-vet/instalacion",
...
if __name__ == "__main__":
    ...
    if sys.version[0] > "2" and sys.version[2] >= "9":
        log(
            ...
            + "."
            + sys.version[2]
            + " Versión correcta de Python... Adelante!"
        )
    else:
        log(
            sys.version[0]
            + "."
            + sys.version[2]
            + " Versión incorrecta de Python... Saliendo"
        )
        sys.exit()
    if is_sudo_user():
